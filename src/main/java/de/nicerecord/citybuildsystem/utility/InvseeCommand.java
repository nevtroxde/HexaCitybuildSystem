...
public class InvseeCommand implements CommandExecutor {
    ...
    public InvseeCommand(CitybuildSystem plugin, MessageManager messageManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("cbsystem.invsee")) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', messageManager.getMessage("general.no-permission")));
            return true;
        }

        if (args.length != 1) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', messageManager.getMessage("commands.invsee.usage")));
            return true;
        }
        ...
        player.sendMessage(ChatColor.translateAlternateColorCodes('&',
            messageManager.getMessage("utility.invsee-opened").replace("%player%", target.getName())));
