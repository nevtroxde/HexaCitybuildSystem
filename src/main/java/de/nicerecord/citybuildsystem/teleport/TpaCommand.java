...
public class TpaCommand implements CommandExecutor {
    ...
    public TpaCommand(CitybuildSystem plugin, TpaManager tpaManager, MessageManager messageManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("cbsystem.tpa")) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', messageManager.getMessage("general.no-permission")));
            return true;
        }

        if (args.length != 1) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', messageManager.getMessage("commands.tpa.usage")));
            return true;
        }
        ...
        boolean success = tpaManager.sendTpaRequest(player, target, TpaRequest.TpaType.TPA);
        if (success) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&',
                messageManager.getMessage("teleport.tpa-sent").replace("%player%", target.getName())));
            target.sendMessage(ChatColor.translateAlternateColorCodes('&',
                messageManager.getMessage("teleport.tpa-received").replace("%player%", player.getName())));
        } else {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', messageManager.getMessage("teleport.tpa-player-disabled")));
        }
